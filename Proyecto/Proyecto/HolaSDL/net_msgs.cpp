...
namespace Net
{
	...
	static void serialise_vec_3f(uint8** buffer, Vector2D v)
	{
		serialise_f32(buffer, v.getX());
		serialise_f32(buffer, v.getY());
	}
	...
	static void deserialise_vec_3f(uint8** buffer, Vector2D* v)
	{
		
		deserialise_f32(buffer, &(v->xf));
		deserialise_f32(buffer, &(v->yf));
	}
